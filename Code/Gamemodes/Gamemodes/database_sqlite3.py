...
class Gamemodes_Database:
    ...
    def _add_gamemode(
        ...
        """
        sql_add_gamemode = 'INSERT INTO gamemodes (name, size, code, watched, random, weighted, equal) VALUES (?, ?, ?, ?, ?, ?, ?)'
    ...
    @staticmethod
    @connection_manager
    def _get_gamemode_id(name: str, cur: sqlite3.Cursor = None) -> int:
        """
        Return the Database's ID of a Gamemode given its `name`.\n
        Do NOT add a `cur` value, its a placeholder which value will be replaced.
        """
        sql_get_gamemode_id = 'SELECT id FROM gamemodes WHERE name = ?'
        gamemode = (name,)
    ...
    @staticmethod
    @connection_manager
    def get_all_gamemodes(cur: sqlite3.Cursor = None) -> list[tuple[str, int, str, bool, bool, bool, bool, int]]:
        """
    ...
    @staticmethod
    @connection_manager
    def delete_gamemode(name: str, cur: sqlite3.Cursor = None) -> None:
        """
        Delete a Gamemode from the Database given its `name`.\n
        Do NOT add a `cur` value, its a placeholder which value will be replaced.
        """
        sql_delete_gamemode = 'DELETE FROM gamemodes WHERE name = ?'
        gamemode = (name,)
    ...
    def edit_gamemode(
        ...
        sql_edit_gamemode = '''
            UPDATE gamemodes
            SET name = ?, code = ?, random = ?, weighted = ?, equal = ?
            WHERE id = ?
        '''
