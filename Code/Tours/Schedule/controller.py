...
class Scheduled_Tour_Controller:
    ...
    def edit_scheduled_tour(self, tour_id: int, description: str = None, host: str = None, timestamp: int = None) -> tuple[bool, str]:
        ...
        try:
            ...
            # Get new values
            new_description = description if description is not None else tour.tour_description
            new_host = host if host is not None else tour.tour_host
            new_timestamp = timestamp if timestamp is not None else tour.starts_at_timestamp
    ...
    def get_all_scheduled_tours(self, guild_id: int) -> list[Scheduled_Tour]:
        """Return a sorted list with all the Scheduled_Tours in the catalog for a specific guild."""
        return [scheduled_tour for scheduled_tour in sorted(self.scheduled_tours.values()) if scheduled_tour.guild_id == guild_id]