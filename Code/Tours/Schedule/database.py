...
class Scheduled_Tours_Database:
    ...
    @staticmethod
    @connection_manager
    def _add_scheduled_tour(description: str, timestamp: str, host: str, cur: sqlite3.Cursor = None) -> None:
        """
        ...
        """
        add_scheduled_tour_query = 'INSERT INTO scheduled_tours (description, timestamp, host) VALUES (?, ?, ?)'
        cur.execute(add_scheduled_tour_query, (description, timestamp, host))

    @staticmethod
    @connection_manager
    def _get_scheduled_tour_id(description: str, timestamp: str, host: str, cur: sqlite3.Cursor = None) -> int:
        """
        Return the Database's ID of a Scheduled_Tour given its `description`, `timestamp` and `host`.\n
        Do NOT add a `cur` value, its a placeholder which value will be replaced.
        """
        get_scheduled_tour_id_query = 'SELECT id FROM scheduled_tours WHERE description = ? AND timestamp = ? AND host = ?'
        cur.execute(get_scheduled_tour_id_query, (description, timestamp, host))
        id = cur.fetchone()[0]
        return id

    @staticmethod
    def add_scheduled_tour(description: str, timestamp: str, host: str, cur: sqlite3.Cursor = None) -> int:
        """
        Add a new Scheduled_Tour to the Scheduled_Tours Database.\n
        Do NOT add a `cur` value, its a placeholder which value will be replaced.
        """
        Scheduled_Tours_Database._add_scheduled_tour(description, timestamp, host)
        return Scheduled_Tours_Database._get_scheduled_tour_id(description, timestamp, host)


    @staticmethod
    @connection_manager
    def get_all_scheduled_tours(cur: sqlite3.Cursor = None) -> list[tuple[str, str, str]]:
        """
